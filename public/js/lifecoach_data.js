...
function lcFillPatientSelect(selectEl, selectedId) {
    ...
    var options = (LC_DATA.PATIENT_OPTIONS || []).length
        ...
        : (LC_DATA.PATIENTS || []).map(function (p) {
...
function lcInitSidebar(activeKey) {
    var map = {
        dashboard:
            (window.LC_ROUTES && window.LC_ROUTES.dashboard) ||
            "/lifecoach/dashboard",
        patients:
            (window.LC_ROUTES && window.LC_ROUTES.patients) ||
            "/lifecoach/patients",
        notes:
            (window.LC_ROUTES && window.LC_ROUTES.notes) || "/lifecoach/notes",
        tasks:
            (window.LC_ROUTES && window.LC_ROUTES.tasks) || "/lifecoach/tasks",
        profile:
            (window.LC_ROUTES && window.LC_ROUTES.profile) ||
            "/lifecoach/profile",
    };
    ...
    document.querySelectorAll(".nav-item[data-page]").forEach(function (btn) {
        btn.classList.toggle(
            "active",
            btn.getAttribute("data-page") === activeKey,
        );
    ...
    if (ham && sidebar) {
        ham.addEventListener("click", function () {
            if (sidebar.classList.contains("open")) lcCloseSidebar();
            else lcOpenSidebar();
        });   
    }

    if (backdrop) {
        backdrop.addEventListener("click", lcCloseSidebar);
    }

    document.addEventListener("click", function (e) {
        var cl = e.target.closest("[data-close]");
        if (cl) {
            lcCloseModal(cl.getAttribute("data-close"));
            return;
        }
