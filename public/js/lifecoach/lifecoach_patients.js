document.addEventListener("DOMContentLoaded", function () {
    ...
    function buildPatientCards() {
        ...
        if (!PATIENTS.length) {
            grid.innerHTML =
                '<div style="padding:24px;color:#94a3b8;font-size:13px;">No patients assigned to you yet. A psychiatrist can assign patients from their workspace.</div>';
            return;
        }
        ...
        PATIENTS.forEach(function (p) {
            ...
            div.innerHTML =
                '<div class="lc-patient-card-accent ' +
                accentClass +
                '"></div>' +
                '<div class="lc-patient-card-body">' +
                '<div class="lc-pc-top">' +
                '<div><div class="lc-pc-name">' +
                lcEscape(p.name) +
                '</div><div class="lc-pc-id">' +
                lcEscape(p.patient_id || p.id) +
                "</div></div>" +
                lcStatusBadge(p.status || "Active") +
                "</div>" +
                '<div class="lc-pc-details">' +
                '<div class="lc-pc-detail-row"><span class="lc-pc-detail-label">Age</span><span class="lc-pc-detail-val">' +
                lcEscape(p.age || "—") +
                "</span></div>" +
                '<div class="lc-pc-detail-row"><span class="lc-pc-detail-label">Chief Complaint</span><span class="lc-pc-detail-val">' +
                lcEscape(p.complaint || "—") +
                "</span></div>" +
                '<div class="lc-pc-detail-row"><span class="lc-pc-detail-label">Next Appointment</span><span class="lc-pc-detail-val">' +
                lcEscape(p.nextAppt || "—") +
                "</span></div>" +
                "</div>" +
                '<button type="button" class="btn-green-full" data-open-patient="' +
                p.id +
                '">View Details</button>' +
                "</div>";
    ...
    document
        .getElementById("lc-patients-grid")
    ...
    window.switchPatientTab = function (tab) {
        document.querySelectorAll(".tab-btn[data-ptab]").forEach(function (b) {
    ...
    document.querySelectorAll(".tab-btn[data-ptab]").forEach(function (btn) {
        btn.addEventListener("click", function () {
            switchPatientTab(this.getAttribute("data-ptab"));
        });
    });
    ...
    function populateDetail(p) {
        ...
        if (infoList) {
            ...
            infoList.innerHTML = fields
                .map(function (f) {
                    return (
                        '<div class="pi-row"><span class="pi-label">' +
                        f[0] +
                        '</span><span class="pi-val">' +
                        f[1] +
                        "</span></div>"
                    );
                })
        ...
        if (rxList) {
            if (!(p.prescriptions || []).length) {
                rxList.innerHTML =
                    '<div style="padding:12px;color:#94a3b8;font-size:13px;">No active prescriptions.</div>';
            } else {
                rxList.innerHTML = p.prescriptions
                    .map(function (rx) {
                        return (
                            '<div class="rx-item"><div class="rx-item-left"><span class="rx-item-tag">' +
                            lcEscape(rx.tag) +
                            '</span><span class="rx-item-name">' +
                            lcEscape(rx.name) +
                            '</span></div><span class="rx-active-badge">Active</span></div>'
                        );
                    })
                    .join("");
            }
        ...
        if (recentNotes) {
            if (!(p.notes || []).length) {
                recentNotes.innerHTML =
                    '<div style="padding:12px;color:#94a3b8;font-size:13px;">No coaching notes yet.</div>';
            } else {
                recentNotes.innerHTML = p.notes
                    .slice(0, 2)
                    .map(function (n) {
                        return (
                            '<div class="pd-note-row"><div class="pd-note-top"><span class="pd-note-type">' +
                            lcEscape(n.type) +
                            '</span><span class="pd-note-date">' +
                            lcEscape(n.date) +
                            '</span></div><div class="pd-note-text">' +
                            lcEscape(n.text) +
                            "</div></div>"
                        );
                    })
        ...
        if (metList) {
            if (!(p.metrics || []).length) {
                metList.innerHTML =
                    '<div style="padding:12px;color:#94a3b8;font-size:13px;">No lifestyle assessment on file.</div>';
            } else {
                metList.innerHTML = p.metrics
                    .map(function (m) {
                        return (
                            '<div class="metric-item"><div class="metric-icon" style="background:#f0fdf4;color:#16a34a;"><i data-feather="' +
                            m.icon +
                            '"></i></div><div class="metric-body"><div class="metric-label-row"><span class="metric-name">' +
                            lcEscape(m.name) +
                            '</span><span class="metric-value ' +
                            m.val +
                            '">' +
                            lcEscape(m.value) +
                            '</span></div><div class="metric-track"><div class="metric-bar ' +
                            m.bar +
                            '" style="width:' +
                            m.pct +
                            '%"></div></div></div></div>'
    ...
    function openGoalModalForEdit(goalId) {
        ...
        if (!p) return;
        var goal = (p.goals || []).find(function (g) {
            return String(g.id) === String(goalId);
        });
        if (!goal) return;

        document.getElementById("goal-title").value = goal.title || "";
        document.getElementById("goal-category").value =
            goal.cat || "Mental Wellness";
    ...
    function resetGoalModal() {
        ...
        ["goal-title", "goal-desc", "goal-date", "goal-progress"].forEach(
            function (id) {
    ...
    function buildGoals(p) {
        ...
        var catColors = {
            ...
            Exercise: "#16a34a",
            Nutrition: "#d97706",
            "Stress Management": "#dc2626",
            "Mental Wellness": "#9333ea",
            "Social Connection": "#0891b2",
        };
        if (!(p.goals || []).length) {
            list.innerHTML =
                '<div style="padding:16px;color:#94a3b8;font-size:13px;">No goals yet. Add one to get started.</div>';
            return;
        }
        p.goals.forEach(function (g) {
            ...
            div.innerHTML =
                '<div class="goal-cat-dot" style="background:' +
                color +
                ';"></div>' +
                '<div class="goal-body">' +
                '<div class="goal-top"><span class="goal-title-text">' +
                lcEscape(g.title) +
                '</span><span class="goal-cat-tag" style="background:' +
                color +
                "18;color:" +
                color +
                ";border:1px solid " +
                ...
                lcEscape(g.date) +
                "</div>" +
                '<div class="goal-progress-row"><div class="goal-prog-track"><div class="goal-prog-bar" style="width:' +
                g.prog +
                "%;background:linear-gradient(90deg," +
                color +
                "," +
                color +
                '88);"></div></div><span class="goal-prog-pct" style="color:' +
                color +
                ';">' +
                g.prog +
                "%</span></div>" +
                ...
                '<button type="button" class="btn-outline-sm" data-edit-goal="' +
                g.id +
                '">Edit</button>' +
                '<button type="button" class="btn-outline-sm danger-outline" data-delete-goal="' +
                g.id +
                '">Delete</button>' +
                "</div>";
    ...
    if (saveGoalBtn) {
        saveGoalBtn.addEventListener("click", function () {
            ...
            if (!title) {
                lcToast("Please enter a goal title.");
                return;
            }
            ...
            if (deleteButton) {
                ...
                lcApi(lcRoute("goalsDestroy", goalId), { method: "DELETE" })
                    .then(function (data) {
                        ...
                        if (p) {
                            p.goals = (p.goals || []).filter(function (g) {
                                return String(g.id) !== String(goalId);
                            });
                            syncGoalHabitData(p);
                            buildGoals(p);
    ...
    function buildHabits(p) {
        ...
        if (!(p.habits || []).length) {
            wrap.innerHTML =
                '<div style="padding:16px;color:#94a3b8;font-size:13px;">No weekly habit goals have been added yet.</div>';
            return;
        }
        ...
        p.habits.forEach(function (habit, hi) {
            ...
            DAYS.forEach(function (d, di) {
                ...
                row +=
                    '<td><input type="checkbox" class="habit-check ' +
                    (checked ? "habit-done" : "habit-miss") +
                    '" ' +
                    (checked ? "checked" : "") +
                    ' data-goal-id="' +
                    ((p.goals[hi] && p.goals[hi].id) || "") +
                    '" data-habit-index="' +
                    hi +
                    '" data-day-index="' +
                    di +
                    '" aria-label="Weekly habit check for ' +
                    lcEscape(habit) +
                    " on " +
                    d +
                    '"></td>';
    ...
    if (habitTable) {
        habitTable.addEventListener("change", function (e) {
            if (
                !e.target.classList ||
                !e.target.classList.contains("habit-check")
            )
                return;
            ...
            lcApi(lcRoute("goalsProgress", goalId), {
                ...
                .then(function (data) {
                    ...
                    var idx = (p.goals || []).findIndex(function (g) {
                        return String(g.id) === String(goalId);
                    });
                    if (idx >= 0) {
                        p.goals[idx] = Object.assign(
                            {},
                            p.goals[idx],
                            liveGoal,
                            {
                                ...
                                weekly_checkins: Array.isArray(
                                    liveGoal.weekly_checkins,
                                )
                                    ? liveGoal.weekly_checkins
                                    : p.habitData[habitIndex],
                            },
                        );
                        p.habitData[habitIndex] = Array.isArray(
                            liveGoal.weekly_checkins,
                        )
    ...
    function buildPatientNotes(p) {
        ...
        if (!(p.notes || []).length) {
            list.innerHTML =
                '<div style="padding:16px;color:#94a3b8;font-size:13px;">No coaching notes yet.</div>';
            return;
        }
        p.notes.forEach(function (n) {
            ...
            div.innerHTML =
                '<div class="gn-top"><span class="gn-patient">' +
                lcEscape(p.name) +
                '</span><div class="gn-meta"><span class="gn-type">' +
                lcEscape(n.type) +
                '</span><span class="gn-date">' +
                lcEscape(n.date) +
                "</span></div></div>" +
                '<div class="gn-text">' +
                lcEscape(n.text) +
                "</div>" +
                '<div class="gn-actions"><button type="button" class="btn-outline-sm" data-delete-note="' +
                n.id +
                '">Delete</button></div>';
            list.appendChild(div);
        });
        ...
        .addEventListener("click", function (e) {
            ...
            lcApi(lcRoute("notesDestroy", id), { method: "DELETE" })
                .then(function (data) {
                    ...
                    if (p) {
                        p.notes = (p.notes || []).filter(function (n) {
                ...
                .catch(function (err) {
                    lcToast(err.message || "Could not delete note.");
                });
        });
    ...
    if (pdAddNoteBtn) {
        pdAddNoteBtn.addEventListener("click", function () {
            lcFillPatientSelect(
                document.getElementById("note-patient"),
                currentPatientId,
            );
    ...
    if (saveNoteBtn) {
        saveNoteBtn.addEventListener("click", function () {
            ...
            if (!patientId) {
                lcToast("Select a patient.");
                return;
            }
            if (!text) {
                lcToast("Please enter a note.");
                return;
            }
